...
export const AuthPage: React.FC = () => {
  ...
  const handleGoogleSignInSubmit = async (e: React.FormEvent) => {
    ...
    const res = await signInWithGoogle(googleEmailInput);
    if (res?.isNewUser) {
          ...
          <div className="bg-white border border-slate-200 rounded-2xl p-6 max-w-sm w-full shadow-2xl space-y-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-[11px] text-slate-500">CuraHome Healthcare Account</p>
              </div>
            </div>

            <form onSubmit={handleGoogleSignInSubmit} className="space-y-3 pt-1">
              <div>
                <label className="block text-xs font-semibold text-slate-700 mb-1">
              ...
              <div className="flex gap-2 pt-2">
                <button
                  ...
                  className="flex-1 py-2 text-xs font-semibold text-slate-600 hover:text-slate-800 rounded-xl border border-slate-200"
                >
                  Cancel
                </button>
                <button
